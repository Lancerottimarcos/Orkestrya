...
/** Mesmo relatório do lado admin, mas escopado à própria sessão de portal - nunca aceita um id de outro cliente. */
export async function GET() {
  const session = await auth();
  if (!session?.user.clientId) return Response.json({ error: "Não autenticado" }, { status: 401 });
  ...
  const [client, metrics, deliveredCards, postStats] = await Promise.all([
    ...
    getClientMetrics(prisma, clientId),
    prisma.kanbanCard.count({ where: { clientId, completedAt: { gte: thirtyDaysAgo } } }),
    prisma.post.groupBy({ by: ["status"], where: { clientId, createdAt: { gte: thirtyDaysAgo } }, _count: { status: true } }),
  ]);
