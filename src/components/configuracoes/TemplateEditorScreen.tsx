...
type Slot = "watermark" | "header" | "footer";
const SLOT_LABELS: Record<Slot, string> = { watermark: "Marca d'água", header: "Cabeçalho", footer: "Rodapé" };
...
function BrandImageSlot({
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/configuracoes/contratos/${templateId}/imagem`, { method: "POST", body: form });
      if (res.ok) {
        const template = await res.json();
        onChange(template[slot === "watermark" ? "watermarkUrl" : slot === "header" ? "headerUrl" : "footerUrl"]);
      }
    } finally {
      setUploading(false);
  ...
  async function handleRemove() {
    onChange(null);
    await fetch(`/api/configuracoes/contratos/${templateId}/imagem?slot=${slot}`, { method: "DELETE" });
  }
...
export function TemplateEditorScreen({ template }: { template: Template }) {
  ...
  const [watermarkUrl, setWatermarkUrl] = useState(template.watermarkUrl);
  ...
  async function handleSave() {
    ...
    setSaved(false);
    try {
      const res = await fetch(`/api/configuracoes/contratos/${template.id}`, {
        method: "PATCH",
    ...
    <div>
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <a href={`/configuracoes/contratos/${template.id}/preview`} target="_blank" rel="noreferrer" title="Mostra a última versão salva, no layout que o cliente vê">
        ...
        <div className="flex flex-col gap-5">
          <Card>
            ...
            <div className="flex flex-col gap-4">
              {CONTRACT_VARIABLE_GROUPS.map((group, i) => (
                <div key={group.key}>
          ...
          <Card>
            <p className="text-sm font-semibold text-ink mb-4">Marca d&apos;água, cabeçalho e rodapé</p>
            <div className="flex flex-col gap-4">
              <BrandImageSlot templateId={template.id} slot="watermark" value={watermarkUrl} onChange={setWatermarkUrl} />
          ...
          <Card>
            ...
            <p className="text-xs text-muted-2 mt-2">
              O modelo padrão é usado automaticamente pela automação de propostas pra gerar o contrato do cliente.
            </p>
          </Card>
        </div>
      </div>
