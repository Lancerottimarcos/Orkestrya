...
const STATUS_COLORS: Record<string, string> = {
  ...
  PAUSED: "var(--color-low)",
  CHURNED: "var(--color-danger)",
};

export function ClientStatusChart({ data }: { data: StatusPoint[] }) {
  const total = data.reduce((sum, d) => sum + d.count, 0);

  if (total === 0) {
    return (
      <div className="h-[200px] flex items-center justify-center">
        <p className="text-xs text-muted">Nenhum cliente cadastrado.</p>
      </div>
    );
  }
                ...
                return (
                  <div className="bg-surface-2 border border-border rounded-lg px-3 py-2 text-xs shadow-xl">
                    <p className="font-semibold text-ink">
                      {CLIENT_STATUS_LABELS[entry.name as string]}: {entry.value as number} ({pct}%)
                    </p>
                  </div>
                );
              }}
      ...
      <div className="flex-1 min-w-[140px] flex flex-col gap-2.5">
        {data.map((entry) => (
          <div key={entry.status} className="flex items-baseline gap-2 text-xs">
            ...
            />
            <span className="text-muted whitespace-nowrap">{CLIENT_STATUS_LABELS[entry.status]}</span>
